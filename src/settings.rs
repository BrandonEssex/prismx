...
// Toggle logic for settings screen
...
pub fn render_settings<B: Backend>(f: &mut Frame<B>, area: Rect, state: &AppState) {
    ...
    for (i, t) in SETTING_TOGGLES.iter().enumerate() {
        ...
        if t.label.starts_with("Theme Preset") {
            label = format!("Theme Preset: {}", current_theme());
        }
        ...
        let mut style = if selected {
            Style::default().fg(theme.focus_outline()).add_modifier(Modifier::BOLD)
        } else {
    ...
    let width = lines.iter().map(|l| l.width() as u16).max().unwrap_or(0).saturating_add(4);
