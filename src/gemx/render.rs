...
pub fn render_gemx<B: Backend>(f: &mut Frame<B>, area: Rect, state: &AppState) {
    ...
    for (&node_id, &Coords { x, y }) in &drawn_at {
        let xs = ((x as f32) * BASE_SPACING_X as f32 * scale) as u16;
        let ys = ((y as f32) * BASE_SPACING_Y as f32 * scale) as u16;
        if ys >= area.height {
            continue;
        }
    ...
    for (source, targets) in &state.link_map {
        for target in targets {
            if let (Some(&Coords { x: sx, y: sy }), Some(&Coords { x: tx, y: ty })) = (drawn_at.get(source), drawn_at.get(target)) {
                let sys = ((sy as f32) * BASE_SPACING_Y as f32 * scale) as u16;
                let sxs = ((sx as f32) * BASE_SPACING_X as f32 * scale) as u16;
                let txs = ((tx as f32) * BASE_SPACING_X as f32 * scale) as u16;
