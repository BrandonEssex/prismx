...
pub fn render_gemx<B: Backend>(f: &mut Frame<B>, area: Rect, state: &AppState) {
    ...
    for (&node_id, &Coords { x, y }) in &drawn_at {
        ...
        let scroll_x = state.scroll_x.max(0);
        let visible_x = x - scroll_x;
        let width = label.len().min((area.width as i16 - visible_x.max(0)) as usize);
        ...
        let draw_x = visible_x.max(0) as u16;
        f.render_widget(para, Rect::new(draw_x, y as u16, width as u16, 1));
    }
            ...
            if let (Some(&Coords { x: sx, y: sy }), Some(&Coords { x: tx, y: ty })) = (drawn_at.get(source), drawn_at.get(target)) {
                if sy == ty && sy < area.height as i16 {
                    ...
                    if mid >= scroll_x && mid < scroll_x + area.width as i16 {
                        ...
                        let draw_x = (mid - scroll_x).max(0) as u16;
                        f.render_widget(para, Rect::new(draw_x, sy as u16, 1, 1));
                    }
                }
            }
        }
